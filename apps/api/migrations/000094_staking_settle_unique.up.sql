-- Idempotent clawback of same-second duplicate daily staking/referral payouts,
-- then unique indexes so a raced settle cannot credit twice.
...
-- Older same-ref credits (e.g. weekly→daily cutover) are not clawed; relabel
-- the later row so the unique indexes can be created.
