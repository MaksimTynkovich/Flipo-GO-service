...
func NewRouter(deps Deps) *gin.Engine {
	...
	v1 := r.Group("/api/v1")
	{
		...
		authed.Use(middleware.JWTAuth(deps.Auth), middleware.UserBanGate(deps.Auth))
		{
			...
			authed.PATCH("/me/locale", deps.AuthHandler.UpdateLocale)
		...
		adminAuthed.Use(middleware.AdminAuth(deps.Auth, deps.AdminTelegramIDs))
		{
			...
			adminAuthed.GET("/users/:id/ledger", deps.AdminHandler.UserLedger)
			adminAuthed.GET("/users/:id/inventory", deps.AdminHandler.UserInventory)
			adminAuthed.GET("/users/:id/case-opens", deps.AdminHandler.UserCaseOpens)
			...
			adminAuthed.POST("/market/bot-listings", deps.AdminHandler.CreateBotMarketListing)
			adminAuthed.GET("/market/stats", deps.AdminHandler.MarketStats)
			...
			adminAuthed.GET("/campaigns", deps.AdminHandler.ListCampaigns)
			adminAuthed.POST("/campaigns", deps.AdminHandler.CreateCampaign)
			adminAuthed.GET("/campaigns/:id", deps.AdminHandler.GetCampaign)
			adminAuthed.PATCH("/campaigns/:id", deps.AdminHandler.PatchCampaign)
			...
			adminAuthed.GET("/staking/stakers", deps.AdminHandler.ListStakingStakers)
			...
			adminAuthed.DELETE("/cases/:id", deps.AdminHandler.DeleteCase)
			...
			adminAuthed.POST("/cases/:id/player-simulate", deps.AdminHandler.PlayerSimulateCase)
			adminAuthed.POST("/cases/player-simulate-all", deps.AdminHandler.PlayerSimulateAllCases)
