...
func (r *UserRepo) AddWagerRequired(ctx context.Context, userID uuid.UUID, amount int64) error {
	if amount <= 0 {
		return nil
	}
	return r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
...
func (r *UserRepo) AddWagerProgress(ctx context.Context, userID uuid.UUID, amount int64) error {
	if amount <= 0 {
		return nil
	}
	return r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		return applyWagerProgressTx(tx, &user, amount)
	})
}
...
func applyWagerRequiredTx(tx *gorm.DB, user *domain.User, amount int64) error {
	...
	required := user.WagerRequiredNanoton + amount
	progress := user.WagerProgressNanoton
	if progress >= required {
		required, progress = 0, 0
	}
...
func applyWagerProgressTx(tx *gorm.DB, user *domain.User, amount int64) error {
	...
	required := user.WagerRequiredNanoton
	if progress >= required {
		required, progress = 0, 0
	}
	user.WagerRequiredNanoton = required
